...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const p = getLegal(locale).legalNotice;
  const first = p.blocks?.[0] && "paras" in p.blocks[0] ? (p.blocks[0].paras?.[0] ?? p.title) : p.title;
  return { ...pageMeta({ locale, path: "/legal-notice", title: p.title, description: clamp(first) }), robots: { index: true, follow: true } };
}

export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const page = getLegal(locale).legalNotice;
  return (
    <>
